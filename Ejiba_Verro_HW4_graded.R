...
#Stuart- I think you meant to put vec1,vec2, but this would still double the 20
vec3 <- c(vec2,vec1)
...
#Stuart- Your answer is correct, but you can solve it much more simply by something like (0.1^(3*1:12)*(0.2^(3*1:12-2)))
...
i = 1:100 #Stuart- This should be 10 to 100, good solution though
...
l = seq(1, length(yVec),1) #Stuart- This should be to length-1 to avoid warnings
trig <- c(sin(yVec[k])/cos(xVec[l])) #Stuart- Be careful, you mixed up which goes in which sequence
print(trig)
...
i = 1: length(xVec)-1 #Stuart- You should have () around len-1 because you wind up getting 0:(len-1)
fun <- function(i) (exp(-xVec[i+1]))
s = sum((fun(i))/(xVec[i]+10)) #Stuart- You should also have () around the interior of the sum, your statement returns a vector
...
print(sam) #Stuart- Your solution is correct, but R allows for a simpler expression. sqrt(abs(xVec-mean(xVec)))
#e How many values in yVec are within 200 of the maximum value of the terms in yVec?
print(length((max(yVec) - 200): max(yVec))) #Stuart- The question is asking for how many values does the vector have in this range
#not the size of the range itself
